...
describe("ReadRawTask", () => {
    describe("non-alphanumeric filenames", () => {
        for (const { str, desc } of _chai_spec_1.NonAlphaStrings) {
            it("reads with " + desc, async () => {
                (0, _chai_spec_1.expect)(ExifTool_1.exiftool.options.useMWG).to.equal(true);
                ...
                const t = await ExifTool_1.exiftool.readRaw(dest);
    ...
    describe("argument-injection hardening", () => {
        it("rejects filename containing a newline (defense-in-depth)", async () => {
            return (0, _chai_spec_1.expect)(ExifTool_1.exiftool.readRaw("input.jpg\n-p\n/etc/passwd\n-w!\nleak.txt")).to.be.rejectedWith(/control character/);
        });
        it("rejects filename containing a NUL byte (defense-in-depth)", async () => {
            return (0, _chai_spec_1.expect)(ExifTool_1.exiftool.readRaw("input.jpg\0-p\0/etc/passwd")).to.be.rejectedWith(/control character/);
        });
    });
});
//# sourceMappingURL=ReadRawTask.spec.js.map